...
```json
{
  ...
  "instructions": [
    ...
    "我的 Obsidian 第二大腦 Vault 位於 D:\\\\Dropbox\\\\Brain，使用 mcpvault (obsidian MCP) 存取。",
    ...
    "當我說「收工」時，請執行：1) 摘要今天變動 2) 更新 D:\\\\Dropbox\\\\Brain\\Teaching-tools\\工作筆記.md 的「上次做到哪」和「最近更動紀錄」 3) git add + commit + push。",
    "當我說「開工」時，請執行：1) 讀 D:\\\\Dropbox\\\\Brain\\Teaching-tools\\工作筆記.md 摘要進度 2) git status 檢查本地狀態 3) git fetch 檢查遠端狀態 4) 建議下一步。"
  ],
  "mcp": {
    "notebooklm-mcp": {
      "type": "local",
