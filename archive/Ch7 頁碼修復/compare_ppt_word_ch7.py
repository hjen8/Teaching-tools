...
WORD_PATH = r"E:\OpenCode\地理(2)學習寶典_08_CH7_臺灣與世界_教用.doc"
REPORT_PATH = r"E:\OpenCode\diff_report_ch7.html"
APPLY_PPT_PATH = r"E:\OpenCode\Ch7_臺灣與世界_更新版.pptx"
MAPPING_PATH = r"E:\OpenCode\section_mapping_ch7.json"
MANUAL_MAPPING_PATH = r"E:\OpenCode\manual_mapping_ch7.json"
...
def run_apply():
    ...
    if word_page:
        ppt_base = detect_ppt_base_page(PPT_PATH)
        if ppt_base:
            offset = word_page - ppt_base
            print(f"PPT 基礎頁碼: {ppt_base}, Word 頁碼: {word_page}, 偏移量: {offset}")
            if offset != 0:
                update_page_numbers(APPLY_PPT_PATH, APPLY_PPT_PATH, offset)
            else:
                print("  偏移量=0，頁碼無需變更")
        else:
            print("[WARN] 無法偵測 PPT 基礎頁碼，跳過頁碼更新")
    else:
...
def run_pagenum():
    ...
    if ppt_base:
        offset = word_page - ppt_base
        print(f"PPT 基礎頁碼: {ppt_base}, Word 頁碼: {word_page}")
        print(f"偏移量: {offset}")
        if offset != 0:
...
def detect_ppt_base_page(ppt_path):
    """自動偵測 PPT 的基礎頁碼（內容起始頁）"""
    pages = find_ppt_page_numbers(ppt_path)
    if not pages:
        return None

    # 按 slide 順序找出第一個出現的頁碼
    # （跳過封面頁，通常從 slide 2 開始）
    min_slide = min(p["slide_idx"] for p in pages)
    first_in_content = None
    for p in sorted(pages, key=lambda x: x["slide_idx"]):
        if p["slide_idx"] >= min_slide + 1:  # 跳過封面
            first_in_content = p
            break

    if not first_in_content:
        first_in_content = pages[0]

    return first_in_content["old_num"]
